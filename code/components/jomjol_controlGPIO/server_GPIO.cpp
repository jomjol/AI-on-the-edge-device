...
void GpioPin::init()
{
    ...
    // set as output mode
    io_conf.mode = (_mode == GPIO_PIN_MODE_OUTPUT) || (_mode == GPIO_PIN_MODE_OUTPUT_PWM) || (_mode == GPIO_PIN_MODE_EXTERNAL_FLASH_PWM) || (_mode == GPIO_PIN_MODE_BUILT_IN_FLASH_LED) ? gpio_mode_t::GPIO_MODE_OUTPUT : gpio_mode_t::GPIO_MODE_INPUT;
...
gpio_pin_mode_t GpioHandler::resolvePinMode(std::string input)
{
    ...
    {
        return GPIO_PIN_MODE_BUILT_IN_FLASH_LED;
    }
    ...
    {
        return GPIO_PIN_MODE_EXTERNAL_FLASH_PWM;
    }
    else if (input == "external-flash-ws281x")
    {
        return GPIO_PIN_MODE_EXTERNAL_FLASH_WS281X;
    }
