...
static uint8_t s_consecutiveAutofocusFailures = 0;
static const uint8_t kMaxAutofocusFailuresBeforeDisable = 3;
...
esp_err_t CCamera::InitCam(void)
{
    ...
    if (s != NULL)
    {
        ...
        case OV5640_PID:
            ...
            CCstatus.CamConfig.CameraFocusEnabled = true; // default autofocus ON for OV5640
            break;
        default:
            ...
            if (s != NULL)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (s_consecutiveAutofocusFailures < 255)
                        {
                            s_consecutiveAutofocusFailures++;
                        }
                        ESP_LOGW(TAG, "AF failed (%u/%u)", (unsigned)s_consecutiveAutofocusFailures, (unsigned)kMaxAutofocusFailuresBeforeDisable);
                        if (s_consecutiveAutofocusFailures >= kMaxAutofocusFailuresBeforeDisable)
                        {
                            CCstatus.CamConfig.CameraFocusEnabled = false;
                            CCstatus.CameraAFInitSuccessful = false;
                            ESP_LOGW(TAG, "Disabling autofocus after repeated failures (possible OV5640 without VCM motor).");
                        }
                        return ret;
                    }
                    ...
                    // AF firmware needs fresh frames while focusing. Poll status
                    // until focus completes or timeout is reached.
                    const uint32_t focusTimeoutMs = 1500;
                    const uint32_t focusPollIntervalMs = 100;
                    const uint32_t maxFocusPolls = focusTimeoutMs / focusPollIntervalMs;

                    uint8_t S_Zone[5] = {0};
                    uint8_t focus_status = FW_STATUS_S_FOCUSING;
                    bool focusCompleted = false;

                    for (uint32_t attempt = 0; attempt <= maxFocusPolls; ++attempt)
                    {
                        camera_fb_t *fb = esp_camera_fb_get();
                        if (fb != NULL)
                        {
                        ...
                        int focus_status_raw = ov5640_autofocus_get_status(s, S_Zone, 5);
                        if (focus_status_raw < 0)
                        {
                            ret = focus_status_raw;
                            ESP_LOGI(TAG, "Read focus status failed: %d", ret);
                            break;
                        }

                        focus_status = (uint8_t)focus_status_raw;
                        ...
                        bool isFocusingStatus = (focus_status <= 0x0F) || ((focus_status >= 0x80) && (focus_status <= 0x8F));
                        if (!isFocusingStatus)
                        {
                            break;
                        }

                        if (attempt < maxFocusPolls)
                        {
                            vTaskDelay(focusPollIntervalMs / portTICK_PERIOD_MS);
                        }
                    }
                    ...
                    if (ret < 0)
                    {
                        ESP_LOGI(TAG, "Focus status failed");
                        if (s_consecutiveAutofocusFailures < 255)
                        {
                            s_consecutiveAutofocusFailures++;
                        }
                    ...
                    else if (focusCompleted)
                    {
                        ...
                        s_consecutiveAutofocusFailures = 0;
                    }
                    else
                    {
                        ESP_LOGI(TAG, "Focus not completed in time or ended with status 0x%02x", focus_status);
                        CCstatus.CameraFocusLevel = ov5640_get_focus_level(s);
                        ESP_LOGI(TAG, "Current focus level: %u", CCstatus.CameraFocusLevel);
                        if (s_consecutiveAutofocusFailures < 255)
                        {
                            s_consecutiveAutofocusFailures++;
                        }
