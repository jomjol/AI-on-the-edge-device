...
#define NR_IE_MIN_LEN (ETH_ALEN + 4 + 1 + 1 + 1)
	...
	while (report_len >= 2 + NR_IE_MIN_LEN)
	{
		...
		ESP_LOGI(TAG, "Roaming: RMM neighbor report bssid=" MACSTR " info=0x%x op_class=%u chan=%u phy_type=%u%s%s%s%s",
...
void wifi_scan(void)
{
	...
	{
		LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Roaming: " + std::to_string(i + 1) + ": SSID=" + std::string((char *)wifi_ap_records[i].ssid) + ", BSSID=" + BssidToString((char *)wifi_ap_records[i].bssid) + ", RSSI=" + std::to_string(wifi_ap_records[i].rssi) + ", CH=" + std::to_string(wifi_ap_records[i].primary) + ", AUTH=" + getAuthModeName(wifi_ap_records[i].authmode));
...
// wifi_config.sta.ft_enabled = 1;	 // 802.11r (BSS Fast Transition) -> Upcoming IDF version 5.0 will support 11r
	...
	strcpy((char *)wifi_config.sta.ssid, (const char *)network_config.ssid.c_str());
	strcpy((char *)wifi_config.sta.password, (const char *)network_config.password.c_str());
	...
	{
		retval = esp_netif_set_hostname(my_sta, network_config.hostname.c_str());
		if (retval != ESP_OK)
		{
	...
	LogFile.WriteToFile(ESP_LOG_INFO, TAG, "WiFi STA init successful done");
	return ESP_OK;
}
