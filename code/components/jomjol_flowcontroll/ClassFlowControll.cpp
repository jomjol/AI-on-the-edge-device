...
bool ClassFlowControll::ReadParameter(FILE *pfile, string &aktparamgraph)
{
    ...
    while (this->getNextLine(pfile, &aktparamgraph) && !this->isNewParagraph(aktparamgraph))
    {
        ...
        else if ((toUpper(splitted[0]) == "DATALOGACTIVE") && (splitted.size() > 1))
        {
        ...
        else if ((toUpper(splitted[0]) == "LOGLEVEL") && (splitted.size() > 1))
        {
        ...
        else if ((toUpper(splitted[0]) == "RSSITHRESHOLD") && (splitted.size() > 1))
        {
        ...
        else if ((toUpper(splitted[0]) == "ESPDEEPSLEEP") && (splitted.size() > 1))
        {
            EspDeepSleepEnabled = alphanumericToBoolean(splitted[1]);
        }
...
esp_err_t ClassFlowControll::GetJPGStream(std::string _fn, httpd_req_t *req)
{
    ...
    else if (_fn == "alg_roi.jpg")
    {
        ...
        if (_send->ImageOkay())
        {
            if (flowalignment)
            {
                flowalignment->DrawRef(_send);
            }
            if (flowdigit)
            {
                flowdigit->DrawROI(_send);
            }
            if (flowanalog)
            {
                flowanalog->DrawROI(_send);
            }
