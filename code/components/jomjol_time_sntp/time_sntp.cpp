...
static void obtain_time(void)
{
    ...
    while (true) {
        ...
        if (retry == retry_count) {
            ESP_LOGW(TAG, "Failed to get time from NTP server after %d attempts! Will check again on next round.", retry); // The NTP client will automatically retry periodically!
            break;
        }
