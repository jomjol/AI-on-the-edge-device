...
//    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Result get_numbers_file_handler: " + string(ret.c_str());
...
esp_err_t get_data_file_handler(httpd_req_t *req)
{
    ...
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Searching data files in /sdcard/log/data");
    ...
    while ((entry = readdir(dir)) != NULL) 
    {
        ...
        LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "File: " + _filename);
        ...
        LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "File Extension: " + _fileext);
...
esp_err_t get_tflite_file_handler(httpd_req_t *req)
{
    ...
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Searching TFLITE in /sdcard/config/");
...
static esp_err_t http_resp_dir_html(httpd_req_t *req, const char *dirpath, const char* uripath, bool readonly)
{
    ...
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Dirpath: '" + string(dirpath) + "'>', Pathlength: " + to_string(dirpath_len));

    /* Retrieve the base path of file storage to construct the full path */
    strlcpy(entrypath, dirpath, sizeof(entrypath));
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "entrypath: '<'" + string(entrypath) + "'>'");
...
/////////////////////////////////////////////////
    if (!readonly) {
        ...
        do {
            ...
            //        LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Chunksize " + to_string() + "", chunksize);
            if (chunksize > 0){
        ...
        fclose(fd);
        //    LogFile.WriteToFile(ESP_LOG_INFO, TAG, "File sending complete");
    }
///////////////////////////////
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (strcmp("wlan.ini", entry->d_name) != 0 )        // wlan.ini soll nicht angezeigt werden!
        {
            ...
            LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Found " + string(entrytype) + ": " + string(entry->d_name) + " (" + string(entrysize) + " bytes)");
...
static esp_err_t send_datafile(httpd_req_t *req, bool send_full_file)
{
    ...
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "uri: " + string(req->uri));

    std::string currentfilename = LogFile.GetCurrentFileNameData();

    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "uri: " + string(req->uri) + ", filename: " + currentfilename + 
...
//    LogFile.WriteToFile(ESP_LOG_INFO, TAG, "Sending file: " + string() + " (" + to_string() + " bytes)...", &filename, file_stat.st_size);
    set_content_type_from_file(req, currentfilename.c_str());

    if (!send_full_file) { // Send only last part of file
        LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Sending last " + to_string(LOGFILE_LAST_PART_BYTES) + " bytes of the actual datafile!");
        ...
        if (fseek(fd, 0, SEEK_END)) {
            ...
            LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "File contains " + to_string(pos) + " bytes");
    ...
    fclose(fd);
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "File sending complete");
...
static esp_err_t send_logfile(httpd_req_t *req, bool send_full_file)
{
    ...
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "uri: " + string(req->uri) + ", filename: " + string(filename) + ", filepath: " + currentfilename);
...
//    LogFile.WriteToFile(ESP_LOG_INFO, TAG, "Sending file: " + string(filename) + " (" + to_string(file_stat.st_size) + " bytes)..");
    set_content_type_from_file(req, filename);

    if (!send_full_file) { // Send only last part of file
        LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Sending last " + to_string(LOGFILE_LAST_PART_BYTES) + " bytes of the actual logfile!");
...
static esp_err_t download_get_handler(httpd_req_t *req)
{
    ...
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "uri: " + string(req->uri) + ", filename: " + string(filename) + ", filepath: " + string(filepath));
...
//                                             req->uri, sizeof(filepath));
    ...
    if (filename[strlen(filename) - 1] == '/') {
        ...
        if (buf_len > 1) {
            ...
            if (httpd_req_get_url_query_str(req, buf, buf_len) == ESP_OK) {
                LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Found URL query => " + string(buf));
                char param[32];
                /* Get value of expected key from query string */
                if (httpd_query_key_value(buf, "readonly", param, sizeof(param)) == ESP_OK) {
                    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Found URL query parameter => readonly=" + string(param));
    ...
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Sending file: " + string(filename) + " (" + to_string(file_stat.st_size) + " bytes)..");
    set_content_type_from_file(req, filename);
    ...
    /* Close file after sending complete */
    fclose(fd);
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "File successfully sent");
...
static esp_err_t upload_post_handler(httpd_req_t *req)
{
    ...
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Receiving file: " + string() + "...", filename);
    ...
    while (remaining > 0) {
        ...
        LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Remaining size: " + to_string(remaining) + " bytes");
    ...
    fclose(fd);
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "File reception complete");
    ...
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Directory: " + directory + ", start_fn: " + to_string(start_fn) + ", found: " + to_string(found));
	directory = directory.substr(start_fn, found - start_fn + 1);
    directory = "/fileserver" + directory;
//    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Directory danach 2: " + string(directory.c_str());
...
/*
    if (strcmp(filepath, CONFIG_FILE) == 0) {
        LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "New config found. Reload handler");
        gpio_handler_deinit();
        MQTTdestroy();
    }
*/
...
//////////////////////////////////////////////////////////////
    ...
    {
        LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Query: " + string(_query));
    ...
    if (_task.compare("deldircontent") == 0)
    {
        ...
        zw = "/sdcard" + zw;
        LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Directory to delete: " + zw);

        delete_all_in_directory(zw);
//        directory = std::string(filepath);
//        directory = "/fileserver" + directory;
        LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Location after delete directory content: " + directory);
        /* Redirect onto root to see the updated file list */
//        httpd_resp_set_status(req, "303 See Other");
//        httpd_resp_set_hdr(req, "Location", directory.c_str());
...
//        return ESP_OK;        
    ...
    else
    {
        ...
        directory = directory.substr(start_fn, found - start_fn + 1);
        directory = "/fileserver" + directory;
        LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Directory danach 4: " + directory);
    }
...
std::string unzip_new(std::string _in_zip_file, std::string _target_zip, std::string _target_bin, std::string _main, bool _initial_setup)
{
    ...
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "miniz.c version: " + string(MZ_VERSION));
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Zipfile: " + _in_zip_file);
    ...
    if (!status)
    {
        LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "mz_zip_reader_init_file() failed!");
        return ret;
    }

    // Get and print information about each file in the archive.
    int numberoffiles = (int)mz_zip_reader_get_num_files(&zip_archive);
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Numbers of files to be extracted: " + to_string(numberoffiles));
    ...
    sort_iter = 0;
    {
        ...
        for (i = 0; i < numberoffiles; i++)
        {
            ...
            p = mz_zip_reader_extract_file_to_heap(&zip_archive, archive_filename, &uncomp_size, 0);
                ...
                LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Raw filename: " + zw);
                ...
                LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Filename to extract: " + zw + ", Temporary filename: " + filename_zw);
    ...
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Success");
    return ret;
}
...
//    static const char* s_Test_archive_filename = "testhtml.zip";
    ...
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "miniz.c version: " + string(MZ_VERSION));
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Zipfile: " + _in_zip_file);
    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Target Dir: " + _target_directory);
    ...
    for (sort_iter = 0; sort_iter < 2; sort_iter++)
    {
        ...
        for (i = 0; i < numberoffiles; i++)
        {
            ...
            LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Filename to extract: " + zw);
            FILE* fpTargetFile = fopen(zw.c_str(), "wb");
            fwrite(p, 1, (uint)uncomp_size, fpTargetFile);
            fclose(fpTargetFile);

            LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "Successfully extracted file \"" + string(archive_filename) + "\", size " + to_string(uncomp_size));
            //            LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "File data: \"" + string() + "\"", (const char*)p);
...
//    LogFile.WriteToFile(ESP_LOG_DEBUG, TAG, "zw: " + string(zw);
